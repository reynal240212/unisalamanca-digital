-- UNI SALAMANCA - SAFE CURRICULUM SYNC (v2)
-- FECHA: 15-04-2026

BEGIN;

-- 0. CREAR NUEVO PROGRAMA RECREACIÓN Y DEPORTES SI NO EXISTE
...
-- ACTUALIZAR DURACIONES DE OTROS PROGRAMAS
UPDATE academic_programs SET duration = '9 Semestres' WHERE id IN ('ef3e7b26-eba6-4d54-ae50-a290020eb3cd', '985a9f98-2191-4652-a485-0354483c0d90', 'af7abacd-c827-44c7-a731-2622761f8cf6', '00c1fecb-2705-4bfa-a7ac-978d01fd2c81');
UPDATE academic_programs SET duration = '6 Semestres' WHERE id IN ('d4c9c4b4-4ccf-4b3f-a2f9-bb79741c58cc', '368a199e-591f-4891-99b7-bf07ad20ad5a', '0ecd7e13-703b-45d8-9f69-41c514cce7bf');

-- 1. CREAR TABLA TEMPORAL DE STAGING
CREATE TEMP TABLE staging_subjects (
    program_id uuid,
    semester integer,
    name text,
    credits integer
...
-- 2. INSERTAR DATOS EN STAGING (ADMINISTRACIÓN)
INSERT INTO staging_subjects (program_id, semester, name, credits) VALUES
...
('00c1fecb-2705-4bfa-a7ac-978d01fd2c81', 5, 'Matemáticas Financieras II (Bis)', 3),
...
('d4c9c4b4-4ccf-4b3f-02f9-bb79741c58cc', 4, 'Bases de Datos II', 3),
...
-- (COMERCIO TECH)
INSERT INTO staging_subjects (program_id, semester, name, credits) VALUES
('0ecd7e13-703b-45d8-9f69-41c514cce7bf', 1, 'Administración Empresarial I', 2),
('0ecd7e13-703b-45d8-9f69-41c514cce7bf', 1, 'Espíritu y Mentalidad Empresarial', 2),
('0ecd7e13-703b-45d8-9f69-41c514cce7bf', 1, 'Microeconomía', 2),
('0ecd7e13-703b-45d8-9f69-41c514cce7bf', 1, 'Introducción al Comercio Internacional', 3),
('0ecd7e13-703b-45d8-9f69-41c514cce7bf', 1, 'Matemáticas Aplicadas', 3),
('0ecd7e13-703b-45d8-9f69-41c514cce7bf', 1, 'Seminario Humanístico I (Formación del Espíritu Científico)', 1),
('0ecd7e13-703b-45d8-9f69-41c514cce7bf', 1, 'Cátedra de Arte, Cultura y Deportes', 1),
('0ecd7e13-703b-45d8-9f69-41c514cce7bf', 1, 'Informática', 2),
('0ecd7e13-703b-45d8-9f69-41c514cce7bf', 1, 'Derecho Constitucional', 2);
-- ... (rest of Comercio semesters omitted for brevity in scratch block but will be in full SQL)

-- 3. ACTUALIZAR MATERIAS EXISTENTES (MERGE)
UPDATE public.subjects s
SET credits = st.credits, semester = st.semester
FROM staging_subjects st
WHERE s.program_id = st.program_id AND s.name = st.name;

-- 4. INSERTAR MATERIAS NUEVAS
INSERT INTO public.subjects (program_id, semester, name, credits)
SELECT st.program_id, st.semester, st.name, st.credits
FROM staging_subjects st
LEFT JOIN public.subjects s ON s.program_id = st.program_id AND s.name = st.name
WHERE s.id IS NULL;

-- 5. LIMPIAR STAGING
DROP TABLE staging_subjects;
